...
class JupyterAPIClient:
    def __init__(self, url: str, kernel_info: Dict[str, Any], headers: Dict[str, str]):
        self._base_url = url
    ...
    def wait_for_ready(self, timeout: float = 0.0):
        start = time.time()
        while True:
            response = self.requests.get(self._kernel_api_base, headers=self._headers)
    ...
    def shutdown(self):
        self.requests.delete(self._kernel_api_base, headers=self._headers)
...
class JupyterAPIManager:
    ...
    def start_kernel(self) -> None:
        ...
        assert "id" in self._kernel_info, (
            "Could not connect to Jupyter Server API. The URL specified may be incorrect."
        )
        self._kernel_api_base = f"{url}/{self._kernel_info['id']}"

    def client(self) -> JupyterAPIClient:
        return JupyterAPIClient(
            url=self._base_url, kernel_info=self._kernel_info, headers=self._headers
        )

    def interrupt_kernel(self) -> None:
        self.requests.post(f"{self._kernel_api_base}/interrupt", headers=self._headers)

    def restart_kernel(self) -> None:
        self.state = RuntimeState.STARTING
        self.requests.post(f"{self._kernel_api_base}/restart", headers=self._headers)
